...
for (var i = 0; i < rows.length; i++) {
  ...
  for (var j = 0; j < cells.length; j++) {
    ...
    if (j == 0)
    {
        ...
        var status_text = rows[i].getElementsByTagName("td")[4].innerHTML;
        var Pstatus_textTD =  parser.parseFromString(status_text, "text/html");
        var bTagByStatus_text = Pstatus_textTD.querySelector('span');
        var text_status_text = bTagByStatus_text.textContent.trim();
        ...
        if (text == trip & date == formattedDate){
            ...
            console.log(text_status_text);
            if(text_status_text == "Закрыт"){
                // Получаем элемент по XPath
                const linkElement = document.evaluate("/html/body/section[1]/div[1]/div[2]/div[2]/div[2]/div/form/a", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
                linkElement.style.backgroundColor = "grey";
            }
...
/*
    if (j == 4)
    {
        var nameTD = cells[j].innerHTML;
        var PnameTD = parser.parseFromString(nameTD, "text/html");
        var bTag = PnameTD.querySelector('span');
        var text = bTag.textContent.trim();
